...
public class Autocarro implements AutocarroMotoristaInterface, AutocarroPassageiroInterface {
     ...
     * <p>
     * Número do passageiro se ocupado. -1 se vazio.
     *
     * @serialField seat
     */
    private int[] seat;
     ...
     *
     * <p>
     * TRUE caso tenha acabado e os passageiros possam sair do autocarro, FALSE
     * caso contrário
     *
     ...
     */
    @Override
    public synchronized void enterTheBus(int ticketID, int passID) {
        //System.out.println("Entering the bus motha focka.Bilhete: " + ticketID + " Bilhetes vendidos: " + bilhetes);
        log.reportState(passID,AuxInfo.passState.TERMINAL_TRANSFER);
        nOcupantes++;
     ...
     */
    @Override
    public synchronized void leaveTheBus(int ticketID,int passID) {
        //System.out.println("IM OUT!Shitty bus");
        log.reportState(passID,AuxInfo.passState.AT_THE_DEPARTURE_TRANSFER_TERMINAL);
        while (!hasEnded) {
            try {
                wait();
