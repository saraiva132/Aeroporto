...
public class TransiçãoAeroporto implements TransicaoPassageiroInterface {
     ...
     */
    public TransiçãoAeroporto(Logging log) {
        this.log = log;
    ...
    public synchronized void goHome(int id) {
        log.reportState(id,passState.EXITING_THE_ARRIVAL_TERMINAL);
    ...
    public synchronized void prepareNextLeg(int id) {
        ...
        log.reportState(id,passState.ENTERING_THE_DEPARTURE_TERMINAL);
        nPassageiros--;
        if (nPassageiros == 0) {
            canLeave = true;
            notifyAll();
